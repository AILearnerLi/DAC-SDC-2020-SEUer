...
//*************************************init *********************************
	...
	data = (ADT*)sds_alloc(4*160*320*sizeof(ADT));
	img = (ADT4*)sds_alloc(4*160*320*sizeof(ADT4));
	img16 = (ADT16*)sds_alloc(160*320*sizeof(ADT16));
	...
	//for (int b = 0; b<4; b++)
	//{
		load_fm(data, config[0]);
		//img_DT_2_DT4(data[0], img, config[0],b);

	//}
	img_DT_2_DT16(data, img16, config[0]);

	//*******************************check 4b image **************************************
//int sum=0;
//	for (int h1=0; h1<160; h1++)
//	{
//		for (int w1=0; w1<320; w1++)
//		{
//			//std::cout<<"img="<<hex<<img[h1*160+w]<<"      img16="<<hex<<img16[h1*160+w1]<<std::endl;
//			if (img[h1*160+w1].range(23,0)!=img16[h1*160+w1].range(95,72))
//			{
...
//	std::cout<<"sum="<<sum<<std::endl;
	...
	SkyNet(img16, ofm_blob32, weight, biasm, layer1_done);
