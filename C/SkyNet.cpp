...
#pragma HLS INTERFACE s_axilite port=layer1_done //bundle=layer_done
...
#pragma HLS ALLOCATION instances=Export_FM1   limit=1 function
    ...
    for(int Hx=0; Hx<4; Hx++)
    {
	    for(int Wx=0; Wx<4; Wx++)
	    {
		    ...
		    for(int b=0;b<4;b++)
			{
				if(b%2==0)
				{
					SHARECONV(FM3, FM4, WBUF[1], 1, 0);
					ACTIVATION(FM4, FM2, BBUF[1], MBUF[1]);
					POOL(fm + pool1_o, FM2, Hx+(b/2)*4, Wx+(b%2)*4, 0, config[3].ow, config[3].oh);

					DFM(FM1,FM2,b+1);

					SHARECONV(FM3, FM4, WBUF[2],1,0);

					ACTIVATION(FM4, FM3, BBUF[2], MBUF[2]);
					POOL(fm + pool1_o, FM3, Hx+(b/2)*4, Wx+(b%2)*4, 1, config[3].ow, config[3].oh);

				}
				else{
					SHARECONV(FM2, FM4, WBUF[1], 1, 0);
					ACTIVATION(FM4, FM3, BBUF[1], MBUF[1]);
					POOL(fm + pool1_o, FM3, Hx+(b/2)*4, Wx+(b%2)*4, 0, config[3].ow, config[3].oh);


					DFM(FM1,FM3,b+1);
					SHARECONV(FM2, FM4, WBUF[2], 1, 0);

					ACTIVATION(FM4, FM2, BBUF[2], MBUF[2]);
					POOL(fm + pool1_o, FM2, Hx+(b/2)*4, Wx+(b%2)*4, 1, config[3].ow, config[3].oh);
				}
			}
	    }
    }
    ...
    std::cout << "DWCONV5+PWCONV5" << std::endl;
    {
        ...
        {
            Load_WBUF3x3(weight + conv9_w, WBUF[0], Nx);
...

    {
        ...
        {
            Load_WBUF3x3(weight + conv11_w, WBUF[0], Nx+24);
